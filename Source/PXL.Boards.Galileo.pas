...

{
  ...
  Thanks to Sergey Kiselev for creating a thorough analysis of Galileo Gen1 GPIO, including an indispensable
  diagram of pin multiplexing on his blog:
      http://www.malinov.com/Home/sergey-s-blog/intelgalileo-programminggpiofromlinux

  Thanks for Emutex Labs for providing a comprehensive diagram of Galileo Gen2 GPIO and multiplexing on their
  web site:
      http://www.emutexlabs.com/project/203-getting-started-with-intel-galileo-gen-2

  Additional information regarding GPIO pins, including multiplexing on Intel Edison, ideas about the data
  structure and finally some meaningful information on Fast I/O were found in "libmraa - Low Level Skeleton
  Library for Communication on GNU/Linux platforms" written by Brendan Le Foll and Thomas Ingleby:
      https://github.com/intel-iot-devkit/mraa
}
{
  Please remember that to use SPI on Intel Galileo Gen2 it is necessary to manually enable chip-select line,
  which is disabled by default for compatibility with Arduino. This can be done by editing "grub.conf" file:
  ...
  After doing this, reboot. It can be verified that SPI chip-select is working by trying to export gpio10,
  which should now produce an error:
...
// Enable the following option to automatically enable Fast I/O on pins that are capable of it, when setting
// their mode to "Output".
{$DEFINE GalileoAutoFastIO}

uses
  PXL.TypeDef, PXL.Boards.Types, PXL.Sysfs.Types, PXL.Sysfs.Core, PXL.Sysfs.GPIO, PXL.Sysfs.PWM,
  PXL.Sysfs.ADC, PXL.Sysfs.SPI, PXL.Sysfs.I2C, PXL.Sysfs.UART;
...
type
  ...
  private
    ...
    procedure SetupMuxes(const APinMuxes: TPinMuxes);
    function GetFastIO: TFastIO;
    function PinToGPIO(const APin: Integer): Integer; inline;
  ...
  protected
    ...
    class procedure InitGalileo1Info(out AInfo: TBoardInfo);
    class procedure InitGalileo2Info(out AInfo: TBoardInfo);
    class procedure InitEdisonInfo(out AInfo: TBoardInfo);
  ...
  public
    ...
    procedure SetFastOutput(const APin: TPinIdentifier);
    procedure SetFastValue(const APin: TPinIdentifier; const AValue: TPinValue);
  ...
  private
    ...
    function PinToPWM(const APin: TPinIdentifier): TPinIdentifier; inline;
  protected
...
var
  LBoardName: StdString;
begin
  inherited Create;

  LBoardName := Trim(ReadTextFromFile('/sys/devices/virtual/dmi/id/board_name'));

  if SameText(LBoardName, 'GalileoGen2') then
    FBoard := TGalileoBoard.Gen2
  else if SameText(LBoardName, 'BODEGA BAY') or SameText(LBoardName, 'SALT BAY') then
    FBoard := TGalileoBoard.Edison
  else if SameText(LBoardName, 'DE3815') or SameText(LBoardName, 'NOTEBOOK') or
    SameText(LBoardName, 'MinnowBoard MAX') then
...
procedure TGalileoGPIO.SetupMuxes(const APinMuxes: TPinMuxes);
var
  I: Integer;
begin
  for I := 0 to Length(APinMuxes) - 1 do
  begin
    FSysfsGPIO.TrySetPinMode(APinMuxes[I].Pin, TPinMode.Output);
    FSysfsGPIO.PinValue[APinMuxes[I].Pin] := APinMuxes[I].Value;
  end;
end;
...
function TGalileoGPIO.PinToGPIO(const APin: Integer): Integer;
begin
  Result := FBoardInfo.Entries[APin].GPIO.Pin;
end;
...
function TGalileoGPIO.GetPinMode(const APin: TPinIdentifier): TPinMode;
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));

  if FPinModes[APin] <> TPinOpMode.GPIO then
    raise EPinNotConfiguredGPIO.Create(Format(SPinNotConfiguredGPIO, [APin]));

  Result := FSysfsGPIO.PinMode[PinToGPIO(APin)];
end;

procedure TGalileoGPIO.SetPinMode(const APin: TPinIdentifier; const AMode: TPinMode);
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));
  ...
  if FPinModes[APin] <> TPinOpMode.GPIO then
  begin
    if FBoardInfo.Entries[APin].GPIO.OutputEnable <> -1 then
    begin
      if AMode = TPinMode.Output then
        FSysfsGPIO.SetMux(FBoardInfo.Entries[APin].GPIO.OutputEnable, TPinValue.Low)
      else
        FSysfsGPIO.SetMux(FBoardInfo.Entries[APin].GPIO.OutputEnable, TPinValue.High);
    end;

    if FBoardInfo.Entries[APin].GPIO.PullupEnable <> -1 then
      FSysfsGPIO.PinMode[FBoardInfo.Entries[APin].GPIO.PullupEnable] := TPinMode.Input;

    SetupMuxes(FBoardInfo.Entries[APin].GPIO.Muxes);
    FPinModes[APin] := TPinOpMode.GPIO;
  end;

{$IFDEF GalileoAutoFastIO}
  if (AMode = TPinMode.Output) and (TPinOpMode.FastIO in FBoardInfo.Entries[APin].Modes) then
    SetFastOutput(APin)
  else
    FSysfsGPIO.PinMode[PinToGPIO(APin)] := AMode;
{$ELSE}
  FSysfsGPIO.PinMode[PinToGPIO(APin)] := AMode;
{$ENDIF}
end;

function TGalileoGPIO.GetPinValue(const APin: TPinIdentifier): TPinValue;
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));
  ...
  Result := FSysfsGPIO.PinValue[PinToGPIO(APin)];
end;

procedure TGalileoGPIO.SetPinValue(const APin: TPinIdentifier; const AValue: TPinValue);
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));

  if FPinModes[APin] = TPinOpMode.FastIO then
    SetFastValue(APin, AValue)
  else if FPinModes[APin] = TPinOpMode.GPIO then
    FSysfsGPIO.PinValue[PinToGPIO(APin)] := AValue
  else
    raise EPinNotConfiguredGPIO.Create(Format(SPinNotConfiguredGPIO, [APin]));
end;

function TGalileoGPIO.GetPinDrive(const APin: TPinIdentifier): TPinDrive;
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));
  ...
  Result := TCustomGPIO(FSysfsGPIO).PinDrive[PinToGPIO(APin)];
end;

procedure TGalileoGPIO.SetPinDrive(const APin: TPinIdentifier; const AValue: TPinDrive);
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));
  ...
  TCustomGPIO(FSysfsGPIO).PinDrive[PinToGPIO(APin)] := AValue;
end;

procedure TGalileoGPIO.SetFastOutput(const APin: TPinIdentifier);
begin
  if APin > Cardinal(Length(FPinModes)) then
    raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [APin]));

  if not (TPinOpMode.FastIO in FBoardInfo.Entries[APin].Modes) then
    raise EPinNotSupportFastIO.Create(Format(SPinNotSupportFastIO, [APin]));

  if FPinModes[APin] <> TPinOpMode.FastIO then
  begin
    SetupMuxes(FBoardInfo.Entries[APin].FastIO.Muxes);
    FPinModes[APin] := TPinOpMode.FastIO;
  end;
...
procedure TGalileoGPIO.SetFastValue(const APin: TPinIdentifier; const AValue: TPinValue);
begin
  if AValue = TPinValue.High then
    PLongWord(FFastIO.Memory)^ := PLongWord(FFastIO.Memory)^ or
      (LongWord(1) shl FBoardInfo.Entries[APin].FastIO.MemBitPos)
  else
    PLongWord(FFastIO.Memory)^ := PLongWord(FFastIO.Memory)^ and
      (not (LongWord(1) shl FBoardInfo.Entries[APin].FastIO.MemBitPos));
end;

class procedure TGalileoGPIO.InitGalileo1Info(out AInfo: TBoardInfo);
var
  I: Integer;
begin
  with AInfo do
...
class procedure TGalileoGPIO.InitGalileo2Info(out AInfo: TBoardInfo);
var
  I: Integer;
begin
  with AInfo do
...
class procedure TGalileoGPIO.InitEdisonInfo(out AInfo: TBoardInfo);
var
  I: Integer;
begin
  with AInfo do
...
function TGalileoPWM.PinToPWM(const APin: TPinIdentifier): TPinIdentifier;
begin
  Result := FGPIO.FBoardInfo.Entries[APin].PWM.Pin;
end;

function TGalileoPWM.GetEnabled(const AChannel: TPinChannel): Boolean;
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
      raise EGPIOInvalidPin.Create(Format(SGPIOSpecifiedPinInvalid, [AChannel]));

    if FPinModes[AChannel] <> TPinOpMode.PWM then
      raise EPinNotConfiguredPWM.Create(Format(SPinNotConfiguredPWM, [AChannel]));

    Result := FSysfsPWM.Enabled[PinToPWM(AChannel)];
  end;
end;

procedure TGalileoPWM.SetEnabled(const AChannel: TPinChannel; const AValue: Boolean);
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
    ...
    if FPinModes[AChannel] <> TPinOpMode.PWM then
    begin
      SetupMuxes(FBoardInfo.Entries[AChannel].PWM.Muxes);
      FPinModes[AChannel] := TPinOpMode.PWM;
    end;

    if not AValue then
      FGPIO.FSysfsGPIO.SetMux(FBoardInfo.Entries[AChannel].GPIO.Pin, TPinValue.Low);

    FSysfsPWM.Enabled[PinToPWM(AChannel)] := AValue;

    if AValue then
      FGPIO.FSysfsGPIO.SetMux(FBoardInfo.Entries[AChannel].GPIO.Pin, TPinValue.High);
  end;
end;

function TGalileoPWM.GetPeriod(const AChannel: TPinChannel): Cardinal;
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
    ...
    Result := FSysfsPWM.Period[PinToPWM(AChannel)];
  end;
end;

procedure TGalileoPWM.SetPeriod(const AChannel: TPinChannel; const AValue: Cardinal);
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
    ...
    FSysfsPWM.Period[PinToPWM(AChannel)] := AValue;
  end;
end;

function TGalileoPWM.GetDutyCycle(const AChannel: TPinChannel): Cardinal;
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
    ...
    Result := FSysfsPWM.DutyCycle[PinToPWM(AChannel)];
  end;
end;

procedure TGalileoPWM.SetDutyCycle(const AChannel: TPinChannel; const AValue: Cardinal);
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FPinModes)) then
    ...
    FSysfsPWM.DutyCycle[PinToPWM(AChannel)] := AValue;
  end;
end;
...
function TGalileoADC.GetRawValue(const AChannel: TPinChannel): Cardinal;
var
  LPin: Integer;
begin
  with FGPIO do
  begin
    if AChannel > Cardinal(Length(FBoardInfo.PWM.Pins)) then
      raise EADCInvalidChannel.Create(Format(SADCSpecifiedChannelInvalid, [AChannel]));

    LPin := FBoardInfo.ADC.Pins[AChannel];

    if FPinModes[LPin] <> TPinOpMode.ADC then
    begin
      SetupMuxes(FBoardInfo.Entries[LPin].ADC.Muxes);
      FPinModes[LPin] := TPinOpMode.ADC;
    end;

    Result := FSysfsADC.RawValue[FBoardInfo.Entries[LPin].ADC.Pin];
  end;
end;
...
begin
  ...
  with FGPIO do
    ...
    begin
      LPin := FBoardInfo.SPI.Pins[I];
      if FPinModes[LPin] <> TPinOpMode.SPI then
        SetupMuxes(FBoardInfo.Entries[LPin].SPI.Muxes);
    end;
...
begin
  ...
  with FGPIO do
    ...
    begin
      LPin := FBoardInfo.I2C.Pins[I];
      if FPinModes[LPin] <> TPinOpMode.I2C then
        SetupMuxes(FBoardInfo.Entries[LPin].I2C.Muxes);
    end;
...
begin
  ...
  with FGPIO do
    ...
    begin
      LPin := FBoardInfo.UART.Pins[I];
      if FPinModes[LPin] <> TPinOpMode.UART then
        SetupMuxes(FBoardInfo.Entries[LPin].UART.Muxes);
    end;
