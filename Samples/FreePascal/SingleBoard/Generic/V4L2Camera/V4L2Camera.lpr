...
uses
  cthreads, SysUtils, PXL.Types, PXL.Surfaces, PXL.ImageFormats, PXL.ImageFormats.FCL, PXL.Timing,
  PXL.Cameras.V4L2;
...
begin
  ...
  try
    ...
    try
      // If you are having problems with the camera, try uncommenting the following line, which would reduce
      // video size to see if it's a bandwidth issue related to USB. Depending on singleboard device and/or
      // camera, not all sizes reported by V4L2 driver are realistically supported, resulting in all kind of
      // weird issues.
